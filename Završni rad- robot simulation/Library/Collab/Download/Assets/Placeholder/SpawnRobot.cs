...
public class SpawnRobot : MonoBehaviour
{
    ...
    private GameObject cloneConsole;

    private Transform cloneTrans;
    private Transform consoleSpawnPos;

    private Vector3 mosPos;
    private int num = 0;
    void Start()
    {
        consoleSpawnPos = GameObject.FindGameObjectWithTag("RobotConsoleSpawn").transform;
    }
    ...
    {
        //spawn a robot
        cloneRobot = Instantiate(robot, mosPos, Quaternion.identity);
        cloneRobot.name = "Robot " + num.ToString(); //name it
        cloneTrans = cloneRobot.GetComponent<Transform>(); //assign its transform
        cloneRobot.GetComponent<RobotManager>().rank = num; //set the rank
        ...
        cloneHandTool.name = "Hand Tool " + num.ToString(); //name it
